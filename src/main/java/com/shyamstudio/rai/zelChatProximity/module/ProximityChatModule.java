  ...
  public void handleChatMessage(@NotNull ChatMessage chatMessage) {
    if (chatMessage.getState() != MessageState.PROCESSING && chatMessage.getState() != MessageState.READY) {
      if (configManager.isDebugEnabled()) {
        plugin.getLogger().info("[DEBUG] Skipping message due to state: " + chatMessage.getState());
      }
      return;
    }
    ...
    if (channelType == ChannelType.STAFF || channelType == ChannelType.PRIVATE) {
      if (configManager.isDebugEnabled()) {
        plugin.getLogger().info("[DEBUG] Skipping proximity filtering for channel type: " + channelType);
      }
      return;
    }
    ...
    PlayerData senderData = dataManager.getPlayerData(senderUUID);
    ...
    if (!senderIsLocal) {
      ...
      if (!anyLocalPlayers) {
        if (configManager.isDebugEnabled()) {
          plugin.getLogger().info("[DEBUG] Sender is GLOBAL and no local players online - no filtering needed");
        }
        return;
      }
    }
    ...
    if (configManager.isDebugEnabled()) {
      plugin.getLogger().info("[DEBUG] Applying proximity filtering - Sender: " + sender.getName() +
      ...
      for (Player onlinePlayer : players) {
        ...
        if (!playerIsLocal) {
          if (!senderIsLocal) {
            ...
            if (configManager.isDebugEnabled()) {
              plugin.getLogger().info("[DEBUG] Added GLOBAL player " + onlinePlayer.getName() + " to viewers (sender is GLOBAL)");
            }
          } else {
            if (configManager.isDebugEnabled()) {
              plugin.getLogger().info("[DEBUG] Excluded GLOBAL player " + onlinePlayer.getName() + " from viewers (sender is LOCAL)");
            }
          }
          continue;
        }
        ...
        if (!playerLocation.getWorld().equals(senderWorld)) {
          if (configManager.isDebugEnabled()) {
            plugin.getLogger().info("[DEBUG] Excluded LOCAL player " + onlinePlayer.getName() + " from viewers (different world)");
          }
          continue;
        }
        ...
        if (distanceSquared <= radiusSquared) {
          filteredViewers.add(onlinePlayer);
          if (configManager.isDebugEnabled()) {
            plugin.getLogger().info("[DEBUG] Added LOCAL player " + onlinePlayer.getName() + " to viewers (2D distance: " + String.format("%.2f", Math.sqrt(distanceSquared)) + ")");
          }
        } else {
          if (configManager.isDebugEnabled()) {
            plugin.getLogger().info("[DEBUG] Excluded LOCAL player " + onlinePlayer.getName() + " from viewers (2D distance: " + String.format("%.2f", Math.sqrt(distanceSquared)) + ")");
          }
        }
      }

      return filteredViewers;
    }).thenAccept(filteredViewers -> {
      plugin.getFoliaLib().getScheduler().runNextTick((task) -> {
        if (filteredViewers.isEmpty()) {
          filteredViewers.add(sender);
          if (configManager.isDebugEnabled()) {
            plugin.getLogger().info("[DEBUG] No players in range, adding sender to viewers");
          }
        }

        if (configManager.isDebugEnabled()) {
          plugin.getLogger().info("[DEBUG] Setting " + filteredViewers.size() + " filtered viewers for " + sender.getName());
        }
        ...
        if (configManager.isDebugEnabled()) {
          plugin.getLogger().info("[DEBUG] Set channel type to CUSTOM for ZelChat recognition");
        }
      });
    });
  }
